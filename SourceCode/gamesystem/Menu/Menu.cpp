...
void Menu::Home_() {
	...
	if (Helper::GetInstance()->FrameCheck(tasks[Home].EaseFrame, 1 / 25.0f)) {

		for (auto i = 2; i < tasks.size(); i++) {
			Helper::GetInstance()->FrameCheck(tasks[i].EaseFrame, 1 / 25.0f);
	...
	tasks[Home].Scl.x = Ease(In, Cubic, tasks[Home].EaseFrame, 0, WINW);
	tasks[Home].Scl.y = Ease(In, Cubic, tasks[Home].EaseFrame, 0, WINH);
