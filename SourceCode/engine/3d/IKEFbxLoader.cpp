...
IKEFBXModel* IKEFbxLoader::LoadModelFromFile(const string& ModelName)
{
    ...
    //モデル生成
    IKEFBXModel* fbxModel = new IKEFBXModel();
...
void IKEFbxLoader::ConvertMatrixFromFbx(DirectX::XMMATRIX* dst, const FbxAMatrix& src)
{
    //行
    for (int i = 0; i < 4; i++) {
        //列
    ...
    if (materialCount > 0) {
        ...
        if (material) {
            if (material->GetClassId().Is(FbxSurfaceLambert::ClassId))
            {
                ...
                IKEFBXModel->ambient.x = (float)ambient.Get()[0];
                IKEFBXModel->ambient.y = (float)ambient.Get()[1];
                IKEFBXModel->ambient.z = (float)ambient.Get()[2];

                //拡散反射光係数
                FbxPropertyT<FbxDouble3> diffuse = lambert->Diffuse;
                IKEFBXModel->diffuse.x = (float)diffuse.Get()[0];
                IKEFBXModel->diffuse.y = (float)diffuse.Get()[1];
                IKEFBXModel->diffuse.z = (float)diffuse.Get()[2];
